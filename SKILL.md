...
**Priority order (ask the strongest first):** **1) Claude-orchestrator (me, highest model)** → **2) codex (highest, high reasoning)** → 3) gemini → 4) `claude` CLI (fresh headless). Claude + codex is a valid council; gemini adds cross-vendor breadth. **cursor and cortex are OUT** (see below) — functional council = **Claude + codex + gemini** (+ fresh `claude` for a blind same-vendor voice / code execution).

| Member                        | Invocation (non-interactive)                                             | Status (verified 2026-07-19)       | Limits / notes                                                                                                                                                                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------ | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Claude** (me, orchestrator) | native, highest model + high/max effort                                  | ✅ functional — anchor + judge     | session limits                                                                                                                                                                                                                                                                          |
...
| **claude** CLI (fresh)        | `claude -p 'P' --model opus`                                             | ✅ functional (~1s)                | **Subscription login — no API key, no separate metering.** Same vendor as the orchestrator (less cross-vendor diversity) but **fresh context = unanchored blind reviewer**, and full **code-capable** agent → the right tool for cursor's intended role + fleet execution.              |
| **cursor** (`agent`)          | `agent -p 'P' --output-format text --model auto`                         | ❌ **needs paid API key**          | Logged in Pro, but **every** run mode returns "Authentication required" — the CLI requires `CURSOR_API_KEY` to execute, and that key is **usage-billed (metered), NOT covered by the flat subscription**. Superseded by the `claude` CLI, which does the same job on subscription auth. |
...
CLIs live at `~/.local/bin/{claude,codex,agent,cortex}` and `~/.nvm/.../bin/gemini`. Auth drifts — re-probe before every real run. Each script bounds every member with a `perl alarm` watchdog so a 429-retry storm or auth stall can't hang the fan-out.
...
- **cursor — needs API key to execute; login is not enough.** `agent status`/`about` confirm the Pro login (even from the tool shell), but running an agent in ANY mode (`-p`, `--mode ask`, non-`-p`) returns "Authentication required". The Cursor CLI gates agent execution behind `CURSOR_API_KEY`; the subscription login only authorizes account display, not runs. Given the no-API-key policy, cursor stays OUT of `council.sh`. Re-enable only if the user carves out an exception and sets `CURSOR_API_KEY`.
