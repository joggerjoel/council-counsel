...
| **cursor** (`agent`)         | `agent -p 'P' --output-format text --model auto`                         | ❌ **needs API key to execute**    | Logged in Pro (`agent status`/`about` confirm), but **every** run mode (`-p`, `--mode ask`, non-`-p`) returns "Authentication required" — the CLI requires `CURSOR_API_KEY` to run agents; subscription login only powers account display. Conflicts with no-API-key policy. |
